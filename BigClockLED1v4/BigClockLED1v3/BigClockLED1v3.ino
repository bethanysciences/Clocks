...
// ** CHANGE BACK TO RTC_DS3231 **
Sodaq_DS3231 RTC;                           // instantiate Chronodot txo time base
...
void setup() {
  ...
  RTC.setEpoch(1572833460);                  // Get UNIX time (seconds since epoch) MST is -6:00 https://www.unixtimestamp.com/index.php and input military time equivalent

// ***Sodaq_DS3231 library does not include the "isrunning()" function
//    if (! RTC.isrunning()) {
...
void loop() {
  ...
  //Comment this out to convert to military time
  if(hour > 12){
    hour = hour - 12;
  }
  
  digit1 = hour/10;               //   02 = 0   18 = 1
  digit2 = hour%10;               //   02 = 2   18 = 8
  ...
  float tempC = (RTC.getTemperature());                       // pull temperature from Chronodot (in C)
...
void showTime(unsigned long value) {
  ...
  for (byte x = 0 ; x < 6 ; x++) {
    ...
    if(x == 2 || x == 4){ //These numbers control the placement of the decimal place
      postNumber(remainder, true);
    }
    else if(x == 5 && remainder == 0){
      postNumber(' ',false);
    }
    else{
